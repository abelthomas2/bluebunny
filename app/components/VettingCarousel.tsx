...
export default function VettingCarousel({ items }: { items: VettingStep[] }) {
  ...
  return (
    <div className="mt-4 mx-auto max-w-4xl px-4">
      ...
      <div className="relative rounded-2xl border border-[#E2EEF5] bg-white shadow-sm min-h-[9rem]">
        ...
        {/* Content */}
        <div className="flex items-start gap-4 px-10 py-7">
          <span className="font-mono text-lg font-bold text-[#5DAFD5] shrink-0">
            {String(idx + 1).padStart(2, '0')}
          </span>
          <div className="min-w-0">
            <p className="font-bold text-[#0C1014]">{item.step}</p>
            <p className="mt-1 text-sm font-mono leading-relaxed text-[#0C1014]">{item.detail}</p>
          </div>
        </div>
